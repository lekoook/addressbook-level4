...
Adding a person to the address book. Prerequisites: Carry out the following commands sequentially:
...
Editing a person in the address book. Prerequisites: There are multiple persons in the contact list.
...
Listing persons in the address book. Prerequisites: There are multiple persons in the contact list.
...
Selecting persons in the address book. Prerequisites: There are at least 7 persons in the contacts list. +
...
Invoking text prediction in the address book. Prerequisites: There are the sample contacts in the address book.
...
Note: Some arguments do not work since they are unsupported. Refer to _Text Prediction_ section in the User Guide for more details.

=== Searching for a person

Finding a person in the address book. Prerequisites: The following commands must be entered, "Alexia Tok" and "Marcus Tok" must exist within the contact list: +
...
Locking and unlocking of the address book. Prerequisites: Address book should not be locked. Check path at `./data/addressbook.xml` to ensure that it is not encrypted: `addressbook.xml.encrypted` and that `addressbook.xml` exists.
...
Deleting a person while all persons are listed. Prerequisites: List all persons using the `list` command. Multiple persons in the list.
...
Updating activities in the schedule. Prerequisites: Carry out the following commands sequentially:
