...
Step 2. The user closes the addressbook.

Step 3. The user re-opens the addressbook. No data will be shown as XML data file is technically not present in the data folder.

Step 4. The user enters the password command with the right password. Addressbook will be refreshed and restored back to its former state (before encryption).
...
** The `PBEKeySpec` is first specified using the "PBEWithMD5AndDES" specification.
** A secret key is generated from `SecretKeyFactory` using "PBEWithMD5AndDES" cipher.
** Then a `Cipher` is used to encrypt or decrypt the file with a given password and key specifications.
...
*Step 3.* If password is acceptable, it is parsed to the `PasswordCommand` object +
...
*Step 5.* If not, it will create a cipher and begin encrypting the address book with the input password. +
...
*Step 6.* Assuming we have encrypted it earlier, isLocked() will return true. +

*Step 7.* A `CommandException` will be thrown to warn the user that the address book is in a locked state.


image::passwordCommand_seq.png[width="600"]
...
* `CommandCompleter#reinitialise()` -- replace all data structure instances with data from the given list of persons.
...
Step 1. User launches the application. `CommandCompleter` is instantiated and will initialise all data structure `Trie` instances with all command keywords and every existing contact’s (aka Person) attributes (name, phone, address, etc.).
...
Step 2. The user inputs `mail` into the command box and executes the command. `execute()` checks if the user system is supported by the Java _Desktop_ library and throws an exception if unsupported. In this case, the mail type is contacts selection type. Hence, `MailCommand#mailToSelection()` is invoked. The Uniform Resource Identifier (URI) is built and `Desktop#mail(URI uri)` is called, which opens up the system’s email application.

Step 3. The user decides to email all contacts with the tag `coworker` and executes `mail t/coworker` in the command box. Similar to Step 2, except that the logic calls `MailCommand#mailToGroups(Model model, Tag tag)` with the input tag as parameter. URI is built with contacts containing the specified tag and the email application opens up.
...
The find function has been revamped to support prioritisation of search similarity. +
...
It also supports searching by other attributes and chaining.
...
`treeMap` was used to store the search results ordered by their Levenshtein distances. +
...
*Step 5.* `FindCommand` creates a `FileEncryptor` object to check if the address book is in a locked state by calling the `islocked()` method. +

*Step 6.* If not, it then proceeds to create `ClosestMatchList` objects. +

*Step 7.* It uses the list of keywords obtained from `ClosestMatchList` to create `AddressContainsKeywordsPredicate` and `TagContainsKeywordsPredicate`. +

*Step 8.* These predicates are combined into a `combinedPredicate` object using the AND operation. +

*Step 9.* The model is then updated by calling `model.updateFilteredPersonList(combinedPredicate)` together with the combined predicate obtained in step 8. +

*Step 10.* A `CommandResult` object will be created and an internal method `findActualMatches` will be called to generate a string of keywords that are exact matches and keywords that are guessed. +

*Step 11.* Model update event will be called and the address book UI will update accordingly. It will also display commandResult based on the `CommandResult` object created in Step 10.
...
[discrete]
=== Use case: Encrypt addressbook
*MSS*

1. User requests to encrypt addressbook with password
2. Addressbook shows encrypted message
+
Use case ends.
...
[none]
* 1a. Addressbook is already encrypted.
+
[none]
...
[discrete]
=== Use case: Decrypt addressbook
*MSS*

1. User requests to decrypt addressbook with password
2. Addressbook shows decrypted message
+
Use case ends.
...
[none]
* 1a. Addressbook is already decrypted.
+
[none]
...
+
[none]
** 1b1. Addressbook will display incorrect password message.
+
Use case ends.
